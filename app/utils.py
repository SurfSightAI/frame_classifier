...
def get_frames(
    spot_prefix: str,
    day_date_range: Tuple[int, int],
    month_date_range: Tuple[int, int],
    ...
    img_blobs = list(
        storage_client.list_blobs(constants.FRAME_DATA_BUCKET, prefix=spot_prefix)
    )
    for blob in img_blobs:
        ...
        if (
            not month_date_range[0] <= int(str_name.split("_")[1]) <= month_date_range[1]
        ):
            continue
        ...
        for index, blob in enumerate(
            list(
                storage_client.list_blobs(
                    ...
                    prefix=classification_map.get(classification_type_prefix),
                )
            )
        )
